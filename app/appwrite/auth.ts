    ...
    try {
        ...
        const { documents } = await  database.listDocuments(
            ...
            [
                ...
                Query.select(['name','email', 'imageUrl', 'joined', 'accountId'])
            ]
        );

        if (documents.length ===0) {
            // User authenticated but not in the database, store user data
...
export const getGooglePicture = async () => {
    try {
        ...
        // Make a request to the Goole People API to get the profile photo
        const response = await fetch(
            'https://poopleapis.com/v1/people/me?personFields=photos',
            {
                headers: {
